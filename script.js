...
let fax = document.getElementById('fax-number');
...
$('#reset').on('click', function() {
    ...
    med.checked = false;
    high.checked = false;
    toastr.info('Form cleared successfully.');
});
...
$('#submit').on('click', function() {
    ...
    for(let x = 0; x < inputs.length; x++) {
        if(inputs[x].value == "" & x!= 8 & x != 9) {
            ...
            toastr.error('Error: ' + names[x] + ' not filled in.');
        }
        else if(x < 6 | x == 10) {
    ...
    if(cost.value < 0 | cost.value == "") {
        toastr.error('Error: Cost must be positive.');
    }
    else if(counter == 0) {
        toastr.options = {
            "positionClass": "toast-bottom-right"
        }
        toastr.success('Form submitted successfully! Thank you.');
    }
});